...
//Прямоугольный треугольник с крестом
class triangle_with_cross : public cross {
protected:
	int quarter; //Четверть координатной плоскости, в которой лежит прямой угол
public:
...
void triangle_with_cross::draw() {
	...
	if (quarter == 3) {
		put_line(nwest(), seast());  //Гипотенуза
		put_line(nwest(), swest()); //Верт. катет
		put_line(swest(), seast()); //Гор. катет
	}
	else if (quarter == 2) {
		put_line(swest(), neast()); //Гипотенуза
		put_line(nwest(), swest()); //Верт. катет
		put_line(nwest(), neast()); //Гор. катет
	}
	else if (quarter == 1) {
		put_line(nwest(), seast()); //Гипотенуза
		put_line(neast(), seast()); //Верт. катет
		put_line(nwest(), neast()); //Гор. катет
	}
	else if (quarter == 4) {
		put_line(swest(), neast()); //Гипотенуза
		put_line(seast(), neast()); //Верт. катет
		put_line(swest(), seast()); //Гор. катет
	}
}
...
//Доп функция присоединения - сверху слева
...
//Доп функция присоединения - сверху справа
